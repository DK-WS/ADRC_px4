...
void PX4CtrlFSM::process()
{
    ...
    case MANUAL_CTRL:
    {
        if (false && rc_dy_data.enter_hover_mode) // Try to jump to AUTO_HOVER
        {
        ...
        else if (true || (param.takeoff_land.enable && takeoff_land_data.triggered && 
        ...
        // cmd触发接收轨迹
            ...
            if ((takeoff_land.delay_trigger.first && now_time > takeoff_land.delay_trigger.second)) {
                ...
                if (param.leader) {
                    ...
                    ROS_INFO_ONCE("\033[32m[px4ctrl] TRIGGER sent, allow user command.\033[32m");    
                }
    ...
    case AUTO_TAKEOFF:
    {
        if ((now_time - takeoff_land.toggle_takeoff_land_time).toSec() < 
            ...
            if (!(state_data.current_state.mode == "OFFBOARD")) {
                ...
                ROS_INFO_ONCE("toggle offboard mode");
            }
    ...
    if (rotor_low_speed_during_land) {// used at the start of auto takeoff
        ...
        controller.ADRC_Control(des, odom_data, imu_data, u);
